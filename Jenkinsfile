...
pipeline {
  ...
  environment {
    ...
    def author = sh """ (#!/bin/bash, returnStdout: true, script: "git --no-pager show -s --format='%an' ${GIT_BRANCH}") """
    //def author = sh """ #!/bin/bash ( returnStdout: true, script: "git show ${commitId} | grep -i Author:") """
    def temp = ''
    }
 stages {
